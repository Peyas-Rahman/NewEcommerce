...
export default function Toast({
  ...
  const error = /unable|failed|error|not found|required|log in|login/i.test(message);
  return <div role="status" className="fixed bottom-5 right-5 z-[120] flex w-[min(380px,calc(100vw-2rem))] items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_18px_50px_rgba(15,23,42,0.18)] animate-[toast-in_.25s_ease-out]">
    ...
    <div className="min-w-0 flex-1"><p className="text-sm font-bold text-slate-900">{error ? "Something went wrong" : title || "Added to cart"}</p><p className="mt-0.5 line-clamp-2 text-xs text-slate-500">{message}</p>{!error && actionHref && <a href={actionHref} className="mt-2 inline-block text-xs font-bold text-orange-600 hover:text-orange-700">{actionLabel}</a>}</div>
